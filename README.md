...
![X Keywords Blocker 插件界面](./images/assets/readme-plugin-interface.jpg)

界面包含批量添加、用户词库和远程词库三个入口。远程词库会按分类展示词条，选择后可以统一提交；用户词库用于查看和管理当前账户已有的屏蔽词。
...
添加或删除完成后，可以打开 [X 屏蔽词设置](https://x.com/settings/muted_keywords) 查看账户当前的屏蔽词列表，确认词条是否已经生效，以及每条词的屏蔽位置、来源和时长参数是否符合预期。
