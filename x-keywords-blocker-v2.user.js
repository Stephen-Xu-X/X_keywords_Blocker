...
(() => {
  ...
  const DEFAULT_CATEGORIES = [
    { name: '常见广告 / 加密 / 赚钱类', descriptions: ['高置信度推广、投资引流和虚假收益表达'], words: ['张雪峰','香港银行卡','deepseek','eSIM','VPN','线下','A股','比特币','夏河','同城','咸鱼','闲鱼','罗永浩','狗','说个暴论','蓝V互粉','蹲个弟弟','涨粉','crypto','giveaway','毫无疑问普通人也可以','这仅仅是开始','每天赚钱','快速获利','暴论','币','春晚','春节'] },
    { name: '色情 / 骚扰 / “福”系评论区常见', descriptions: ['高风险词，请按自己的使用场景谨慎选择'], words: ['少妇','看片','福利','日本美女','AI裸照','搭子','弟弟','附近','墨迹','勿扰','小狗','小姐','主人','资','万达广场','福','比她好看的没她骚','比她骚的没她好看','比我骚的没我好看','体制内老师','同城上门','玩归玩闹归闹','只入身体','刷了半天的','就她的主页能打','我福不黑不信你看','应该没人比我玩的开了吧','有人想锐评一下我的福','她太涩了','我真顶不住'] },
  ];

  const state = {
    activeView: 'add',
    presetCategories: DEFAULT_CATEGORIES,
    presetSource: '内置词库',
    presetSyncedAt: '',
    presetStats: { categories: 2, words: DEFAULT_CATEGORIES.reduce((sum, item) => sum + item.words.length, 0), duplicates: 0, invalid: 0 },
  ...
  function parseKeywordMarkdown(markdown) {
    ...
    for (const rawLine of String(markdown).split(/\r?\n/)) {
      ...
      if (!current) continue;
      if (line.startsWith('#### ')) {
        current.descriptions.push(line.slice(5).trim());
        continue;
      }
      if (!line) continue;
  ...
  function install() {
    ...
    function renderPresets() {
      presets.innerHTML = state.presetCategories.map((category) => `<section class="xmks-category"><div class="xmks-category-head"><div><div class="xmks-category-title">${escapeHtml(category.name)}</div>${category.descriptions?.length ? `<div class="xmks-category-desc">${escapeHtml(category.descriptions.join(' · '))}</div>` : ''}</div><div class="xmks-category-count">${category.words.length} 个</div></div><div class="xmks-chips">${category.words.map((word) => `<button class="xmks-chip" type="button" data-word="${escapeHtml(word)}" data-selected="${state.presetSelected.has(word)}" aria-pressed="${state.presetSelected.has(word)}">${state.presetSelected.has(word) ? icon('check',15) : ''}${escapeHtml(word)}</button>`).join('')}</div></section>`).join('');
      updateHeader();
    }
